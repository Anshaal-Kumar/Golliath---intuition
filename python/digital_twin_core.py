    ...
    def clean_and_process(self, df):
        ...
        for column in processed_df.columns:
            if processed_df[column].dtype in ['int64', 'float64']:
                ...
                processed_df[column].fillna(
                    processed_df[column].mode().iloc[0] if not processed_df[column].mode().empty else 'Unknown', 
                    inplace=True
                )
...
class OntologyEngine:
    ...
    def build_knowledge_graph(self, df, insights):
        """Build knowledge graph from data and insights"""
    ...
    def _save_ontology(self):
        ...
        for entity_name, entity_data in self.ontology['entities'].items():
            cursor.execute('''
                ...
                VALUES (?, ?, ?)
            ''', (entity_data['type'], entity_name, json.dumps(entity_data)))
...
class SimulationEngine:
    ...
    def run_what_if_analysis(self, target_column, feature_changes):
        ...
        return {
            ...
            'impact': impact,
            'percent_change': (impact / baseline_prediction * 100) if baseline_prediction != 0 else 0,
            ...
            if column != target_column:
                ...
                if abs(risk_mean - normal_mean) > 0.1 * normal_mean:
                    risk_factors[column] = {
                        ...
                        'high_risk_average': risk_mean,
                        'risk_multiplier': risk_mean / normal_mean if normal_mean != 0 else 1
                    }
        
        return {
            'risk_threshold': risk_threshold,
            'high_risk_cases': len(high_risk_data),
            'risk_factors': risk_factors,
            'risk_probability': len(high_risk_data) / len(df) * 100
